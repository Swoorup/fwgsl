...
fn render_block_pipeline_compiles_to_valid_wgsl() {
    ...
    let mir =
        shadml_mir::lower::lower_hir_to_mir(&arena, &hir).expect("MIR lowering should succeed");

    // Assert MIR contains the expected render block
    assert_eq!(
        mir.render_blocks.len(),
        1,
        "expected exactly one render block"
    );
    ...
    assert!(
        ...
        "WGSL should contain @vertex, got:\n{}",
        wgsl
    );
    assert!(
        wgsl.contains("@fragment"),
        "WGSL should contain @fragment, got:\n{}",
        wgsl
    );
    assert!(
        wgsl.contains("fn vsMain("),
        "WGSL should contain vertex entry, got:\n{}",
        wgsl
    );
    assert!(
        wgsl.contains("fn fsMain("),
        "WGSL should contain fragment entry, got:\n{}",
        wgsl
    );
    assert!(
...
fn render_block_bundler_produces_entries_with_render_block() {
    ...
    assert_eq!(
        profile.entries.len(),
        2,
        "expected two split entries (vertex + fragment)"
    );
    ...
    assert!(
        vertex_entry
            .bind_groups
            .iter()
            .any(|bg| { bg.bindings.iter().any(|b| b.name == "globals") }),
...
"#,
    ...
    let rust_source =
        std::fs::read_to_string(&output_path).expect("should read generated rust source");
